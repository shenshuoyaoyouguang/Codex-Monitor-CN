...
type MockTranslationValue =
  | string
  | ((options?: Record<string, unknown>) => string);

const { mockChangeLanguage, mockT } = vi.hoisted(() => {
  const translations: Record<string, MockTranslationValue> = {
    ...
    "home.resets": "Resets",
    "home.resetsAt": (options) => `Resets ${String(options?.relative ?? "").trim()}`.trim(),
    ...
    "time.daysAgo": (options) => `${options?.count} days ago`,
    "time.hoursAgo": (options) => `${options?.count}h ago`,
    "time.minutesAgo": (options) => `${options?.count}m ago`,
    "time.minutesFromNow": (options) => `in ${options?.count}m`,
    "time.secondsAgo": (options) => `${options?.count}s ago`,
    "time.secondsFromNow": (options) => `in ${options?.count}s`,
    "time.now": "now",
  };

  const hoistedMockT = (key: string, options?: Record<string, unknown>) => {
    const value = translations[key];
    return typeof value === "function" ? value(options) : value ?? key;
  };
