...
export function formatRelativeTime(date: Date | number): string {
  ...
  const format = (value: number, unit: Intl.RelativeTimeFormatUnit) =>
    new Intl.RelativeTimeFormat(i18next.language || 'en', {
      numeric: 'always',
      style: 'long',
    }).format(value, unit);
  ...
  const diffWeeks = Math.round(diffSeconds / (60 * 60 * 24 * 7));
  if (absSeconds < 60 * 60 * 24 * 30) {
    return format(diffWeeks, 'week');
  }

  const diffMonths = Math.round(diffSeconds / (60 * 60 * 24 * 30));
  if (absSeconds < 60 * 60 * 24 * 365) {
    return format(diffMonths, 'month');
  }

  const diffYears = Math.round(diffSeconds / (60 * 60 * 24 * 365));
